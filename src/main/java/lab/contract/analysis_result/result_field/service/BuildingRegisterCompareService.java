...
public class BuildingRegisterCompareService {
    ...
    public void saveBuildingRegisterComment(Long contractId) {
        ...
        ResultField resultField = ResultField.builder()
                ...
                .type(result1.result)
                .build();
        allResult.addBuildingField(resultField);
        ...
        ResultField resultField2 = ResultField.builder()
                ...
                .type(result2.result)
                .build();
        allResult.addBuildingField(resultField2);
        ...
        ResultField resultField3 = ResultField.builder()
                ...
                .type(result3.result)
                .build();
        allResult.addBuildingField(resultField3);
    ...
    public Result compareAddress(Contract contract) {
        ...
        if (addressMatcher.find()) {
            ...
            if (cleanContractAddress.contains(cleanBuildingStreetAddress)) {
                ...
                //rate(STRONG);
                RATE -= 33;
                comment.append("건축물대장과 계약서의 소재지 주소(도로명주소)가 일치하지 않습니다.\n\n" +
                        "계약서에 작성한 정보가 건축물대장과 다를 시 임차인이 저곳에 산다는 권리를 인정받기 어렵습니다.\n\n" +
                        "임대차계약서를 다시 작성해주세요.\n\n\n");
            }
        } else {
            ...
            if (cleanContractAddress.contains(cleanBuildingRegisterAddress)) {
                ...
                //rate(STRONG);
                RATE -= 33;
                comment.append("건축물대장과 계약서의 소재지 주소(지번)가 일치하지 않습니다.\n\n" +
                        "계약서에 작성한 정보가 건축물대장과 다를 시 임차인이 저곳에 산다는 권리를 인정받기 어렵습니다.\n\n" +
                        "임대차계약서를 다시 작성해주세요.\n\n\n");
            }
        }
    ...
    public Result compareRentalPart(Contract contract) {
        ...
        if(rentalPartMatcher.find()) {
            ...
            if (cleanContractRentalPart.contains(cleanBuildingStreetAddress)) {
                ...
                //rate(STRONG);
                RATE -= 8;
                comment.append("건축물대장과 계약서의 임대할 부분이 일치하지 않습니다.\n\n" +
                        "계약서에 작성한 정보가 건축물대장과 다를 시 임차인이 저곳에 산다는 권리를 인정받기 어렵습니다.\n\n" +
                        "임대차계약서를 다시 작성해주세요.\n\n\n");
            }
        } else {
            ...
            if (cleanContractRentalPart.contains(cleanBuildingRegisterAddress)) {
                ...
                comment.append("건축물대장과 계약서의 임대할 부분이 일치하지 않습니다.\n\n" +
                        "임대하는 곳이 토지인가요?\n\n" +
                        "토지가 아니면 일반적으로 도로명주소로 적는 것이 적합합니다.\n\n" +
                        "또한, 계약서에 작성한 정보가 건축물대장과 다를 시 임차인이 저곳에 산다는 권리를 인정받기 어렵습니다.\n\n" +
                        "임대차계약서를 다시 작성해주세요.\n\n\n");
            }
        }
        ...
        if (containsCleanContractRentalPart) {
            ...
            result.setResult(STRONG);
            comment.append("건축물대장의 명칭이 임대할 부분에 포함되어있지 않습니다.\n\n" +
        ...
        if(cleanContractRentalPart.contains(buildingHoTitle)) {
            ...
            //rate(STRONG);
            RATE -= 8;
            comment.append("건축물대장의 호명칭이 임대할 부분에 포함되어있지 않습니다.\n\n" +
                    "상세주소가 있는 경우 동, 층, 호를 명확하게 기재해야 합니다.\n\n\n");
        }
        ...
        if(sharerPart == 0 || ownerPart > sharerPart){
            ...
            if(contractName.equals(ownerName)) {
                ...
                comment.append( "소유권 지분이 낮은 사람과 계약서를 작성했을 경우 해당 임대차계약은 무효이며 우선변제권을 갖지 못합니다.\n\n" +
                        ...
                        "계약서의 임대인과 건축물대장의 소유자 이름이 일치하지 않습니다.\n\n\n");
            }
            if(contractAddress.equals(extractOwnerAddress)){
                ...
                result.setResult(NORMAL);
                RATE -= 12;
                comment.append("계약서의 임대인과 건축물대장의 소유자 주소가 일치하지 않습니다.\n\n");
            }
            if(contractResidentNumber.equals(ownerResidentNumber)) {
                ...
                result.setResult(STRONG);
                RATE -= 11;
                comment.append("계약서의 임대인과 건축물대장의 소유자 주민등록번호가 일치하지 않습니다.\n\n");
                comment.append("임대인의 주민등록증을 확인해보세요. 계약서 상의 임대인의 정보와 일치하는지 확인하세요.\n\n\n");
            }
            ...
            if(contractName.equals(ownerName) || contractName.equals(sharerName)) {
                ...
                result.setResult(STRONG);
                RATE -= 12;
                comment.append("계약서의 임대인과 건축물대장의 소유자 이름이 일치하지 않습니다.\n\n");
            }
